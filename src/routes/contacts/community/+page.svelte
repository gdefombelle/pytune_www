<script lang="ts">
  ...
  import { joinCommunityRequest } from '../../../services/contactService';
  // On ne garde que le composant combiné
...
let numberOfPianos = '';
  ...
  // Variables liées au Super-Composant
  let address = '';
  let phone = '';
  ...
  async function submit() {
    ...
    // 1. Validation du téléphone avant envoi
  ...
  <div class="pytune-card contact-card">
    ...
    {#if !submitted}
      <div class="hero">
        ...
        <p class="hero-sub">
          PyTune offers churches, choirs and nonprofit organizations
          access to all major professional features —
          freely provided for community use, within reasonable needs.
        </p>
      </div>

      <div class="collab-note">
        <p>
          We are also happy, if appropriate, to reference your concerts
          or initiatives on our community page.
        </p>
      ...
      <p class="foundation-note">
        We believe that music in communities deserves
      ...
      <div class="form">
        ...
        <input placeholder="Organization name"
          bind:value={organization} required />
      ...
      <div class="grid">
        <input
          type="number"
          min="1"
          placeholder="Approximate number of pianos"
          bind:value={numberOfPianos}
        />

        <input
          placeholder="Instagram / Social media (optional)"
          bind:value={instagram}
        />
      </div>

      <input
        placeholder="YouTube or Facebook page (optional)"
        bind:value={youtube}
      />
        ...
        <textarea
          placeholder="Tell us about your instruments, venues, number of pianos, musicians involved, and any relevant details."
          bind:value={message}
        />
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? 'Sending...' : 'Request Free Access'}
        </button>
...
.collab-soft {
  ...
  .contact-root {
    ...
    background:
      ...
      url('/assets/background-church-3.webp');
